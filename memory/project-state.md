...
- Ближайший доменный инкремент уточнен: persisted edit slice в `WorkOrder` / `EditVersion` через `UpdateEditVersionFeatureGeometry` для одной line feature и одной внутренней вершины, immutable baseline/current snapshots, AOI `CoveredBy`, atomic guards, `DraftVersionToken`, обязательный `CommandId`, command response + durable readback и revert. Review/post идет только после устойчивого change set.
- Последний RAW ingest: 2026-07-25, `RAW_inputs/meetings/first_save_for_edit_version.md`.
...
## Состояние Wiki На 2026-07-25

- RAW inputs: 36 файлов, все отражены в `RAW_inputs/index.md`; новых и необработанных RAW inputs нет.
...
- Последний lint: 2026-07-25. Обнаружены только 35 ожидаемых `missing_frontmatter` в неизменяемых RAW Markdown files; новых lint-проблем вне `RAW_inputs/` нет. Follow-up `FU-2026-06-01-004` остается открытым.
...
- First-save model имеет один неблокирующий implementation follow-up: precision grid, `CommandId` retention/fingerprint, `BaselineRevisionRef` mapping и event evidence.
