...
| [[Wiki/specifications/edit_version_has_persisted_change_set]] | В `EditVersion` есть saved diff относительно baseline; первый минимум - geometry diff существующей line feature, доказанный readback persisted object + baseline diff. | Workspace пока read-only или UI-only draft; submit/review преждевременны. | high | `RAW_inputs/meetings/persisted_edit_slice_EditVersion.md`; `RAW_inputs/meetings/persisted_edit_slice_for_edit_version.md` |
| [[Wiki/specifications/edit_version_basic_draft_validation]] | First save синхронно подтверждает `geometryValid`, `aoiOk`, `associationsUnchanged`, `topologyNotChecked`, `dirtyRelativeToBaseline` и optional `concurrencyOk`. | Draft change set не годится для следующего workflow step. | high | `RAW_inputs/meetings/persisted_edit_slice_EditVersion.md`; `RAW_inputs/meetings/persisted_edit_slice_for_edit_version.md` |
