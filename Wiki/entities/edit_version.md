...
confidence: high
related: [Wiki/entities/work_order, Wiki/entities/default_state, Wiki/glossary/base_work_state, Wiki/commands/open_edit_version, Wiki/commands/update_edit_version_feature_geometry, Wiki/value_objects/draft_version_token, Wiki/value_objects/command_id, DDD_Wiki/aggregates/edit_version]
---
...
Изолирует рабочий контекст одной `WorkOrder` от authoritative `Default`. Создается как deep copy активного `DefaultState`. Для first save агрегат хранит единое immutable [[Wiki/glossary/base_work_state]] и full current snapshot одной изменяемой line `NetworkFeature`; current diff, `operation` и `hasPersistedChangeSet` вычисляются относительно этого состояния. Persisted draft сопровождается `DraftVersionToken` и `basicValidation`.

## Invariants

- Принадлежит одному `WorkOrder` и назначенному `Editor`.
- `baseNetworkRevision` фиксируется при создании и не меняется.
- Все features одной назначенной работы сравниваются с одним базовым состоянием работы; их собственные версии описывают историю объектов, а не отдельные baseline.
