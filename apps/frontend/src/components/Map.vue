...
function initLayers(){
  ...
  if(points_clusters){
    ...
    map.addLayer({
      ...
      filter: ["has", "point_count"],
      paint: {
        "circle-radius" : ["interpolate", ["linear"], ["get", "point_count"], 10, 12, 100, 20],
        "circle-opacity": 0.8
      }
    }, "points_unclustered");
  }
...
function bindInteractions(){
  ...
  map.on("mouseenter", "points_clusters", () => map!.getCanvas().style.cursor = "pointer");
  map.on("mouseleave", "points_clusters", () => map!.getCanvas().style.cursor = "");
