...
confidence: high
related: [Wiki/glossary/utility_gis_editing, Wiki/commands/open_edit_version, Wiki/commands/update_edit_version_feature_geometry, Wiki/commands/post_to_default]
---
...
Sprint 1 покрывает login, список назначенных `WorkOrder`, открытие `EditVersion` и загрузку workspace. Следующий code-aware slice должен расширить этот путь до edit-save-readback: сдвиг внутренней вершины существующей line feature, сохранение geometry diff относительно baseline, readback persisted feature + explicit baseline diff и basic draft validation flags.

## Planned Slices

Следующий integrated review/post путь должен быть встроен в текущий `WorkOrder` / `EditVersion` flow, но только после persisted change set. Сначала: workspace -> `UpdateEditVersionFeatureGeometry` -> persisted geometry diff existing line -> readback persisted object + baseline diff -> basic draft validation. Затем: `submit_for_review`, package build, reviewer decision, computed `can_post`, simulated post и durable audit. Планирование идет маленькими спринтами; старый standalone Release 2 implementation contract остается reference/legacy source, а не текущим implementation source of truth.
