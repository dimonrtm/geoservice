...
```text
workspace_loaded -> feature_update_persisted -> draft_validated -> submit_for_review -> draft_package -> ready_for_review -> under_review -> approved | returned | escalated -> can_post | blocked_post | stale -> simulated_posted
```
...
Новые durable review states не добавляются до появления реального submit/reviewer backend. До этого readiness выражается computed predicates поверх persisted change set, draft validation, stale marker и editor summary/evidence.
