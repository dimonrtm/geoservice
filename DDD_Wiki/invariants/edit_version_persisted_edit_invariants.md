...
confidence: high
related: [Wiki/entities/edit_version, Wiki/glossary/base_work_state, Wiki/glossary/coordinate_storage_precision, Wiki/policies/edit_geometry_precision_policy, Wiki/policies/positional_accuracy_acceptance_policy, Wiki/commands/update_edit_version_feature_geometry, Wiki/specifications/edit_version_has_persisted_change_set, Wiki/specifications/edit_version_basic_draft_validation, DDD_Wiki/state_machines/edit_version_save_request]
---
...
- Отсутствие утверждённой спецификации или independent positional evidence даёт `POSITIONAL_ACCURACY_UNVERIFIED`: technical save допустим, но review/completion/post заблокированы.
- Dataset precision берётся из фактических spatial-reference metadata сохраняющего dataset. `XY tolerance`, display precision и величина перемещения не подменяют acceptance tolerance.
...
- Одинаковый `CommandId` и fingerprint выполняют не более одной operation; concurrent retry получает тот же pending или terminal outcome. Domain rejection резервирует id, исправленный intent требует нового id.
...
Проверка позиционной точности по утверждённой спецификации и evidence, topology validation, subnetwork cleanliness, full association-row diff, explicit review diff summary, reconcile against newer `Default`, review evidence, `can_post` и authoritative post выполняются downstream и не входят в technical first persisted edit slice.
