...
Do not add `session.scalar(...)` checks between the first and second
`open_for_work_order(...)` calls. SQLAlchemy starts an implicit transaction for
those reads, while `EditVersionService` starts its own explicit transaction with
`session.begin()`.
